...
def handle_uploaded_file(f, user_obj):	
	...
	with open('templates/kpi_app/a.csv', 'r') as destination:
		...
		for row in x:
			dim_obj = Dimension.objects.get_or_create(dim_type= row['Dimension'], company_name = company_obj)[0]
			print(dim_obj)
			break
		for row in x:
			dimValue1_obj = DimensionValue.objects.get_or_create(dim_type_id = dim_obj, dim_name = row['DimName'], level=row['Level'], parent=DimensionValue.objects.get(id=row['ParentId']))[0]
